#!/usr/bin/env python3
"""
ingest_keymetrics.py

Descarga key metrics TTM desde FMP para todos los tickers de
universos.stock_opciones_2026 e inserta en ingest.keymetrics.

Endpoint: https://financialmodelingprep.com/stable/key-metrics?symbol={ticker}&apikey={API_KEY}
Respuesta: lista — se toma el primer elemento (dato más reciente / TTM).

Ejecución: python micro/ingest/ingest_keymetrics.py
"""
...
LOG_FILE = os.path.join(LOG_DIR, f"ingest_keymetrics_{date.today().isoformat()}.log")
...
# ── API ────────────────────────────────────────────────────────────────────────
def fetch_keymetrics(ticker: str) -> dict | None:
    """
    Llama al endpoint FMP stable/key-metrics.
    La respuesta es una lista; se toma el primer elemento (TTM).
...
def main():
    ...
    print(f"\n{'='*65}")
    print(f"  INGEST KEY METRICS — {datetime.now().strftime('%Y-%m-%d %H:%M')}")
